...
public class GestorHorarisTransports implements Serializable {
	...
	public static ArrayList<String> obtenirHoresParada(int pIdLinia, int pIdSentido, int pIdJornada, int pIdParada) {
		//	obtenirLinies
		Thread thread = new Thread(){
			public void run(){
				 ...
				 */
				String contingut = getContingutURL(CTE_OBTENIR_HORARIS_PARADA.
	...
	private static ArrayList<String> parserObtenirHoresParada(String pContingut) {
		/*
		 	<td style="border-bottom:5px solid transparent;text-align:center;color:black;" class="cosTaula text02">
				 <div style="background:#d8d9da;">&nbsp;6:43</div>
			</td>
		 */
		ArrayList<String> arrHores = new ArrayList<String>();
		//
		int ind1 = 0;
		int ind2 = 0;
		do {
			try {
				ind1 = pContingut.indexOf("<div style=\"background:#d8d9da;\">&nbsp;", ind2) + "<div style=\"background:#d8d9da;\">&nbsp;".length();
				if (ind1 < ind2)
					break;
				ind2 = pContingut.indexOf("</div>", ind1);
				//
				String hora = tractarHora(pContingut.substring(ind1, ind2));
				if (hora == null)
					break;
				arrHores.add(hora);
			} catch (Exception ex) {	
	...
	public static ArrayList<String> obtenirTempsEspera(int pIdLinia, int pIdSentido, int pIdJornada, int pIdParada) {
		//	obtenirLinies
		Thread thread = new Thread(){
			public void run(){
	...
	protected static ArrayList<String> parserObtenirTempsEspera(String pContingut) {
		/*
			<tr style='background-color:#fff299;'>
				<td class="quantTrigaraLeft"></td>
		 ...
		 */
		ArrayList<String> arrTempsEspera = new ArrayList<String>();
		//
		int ind0 = 0;
		int ind1 = 0;
		int ind2 = 0;
		do {
			try {
				ind1 = pContingut.indexOf("<td class=\"quantTrigaraCenter\">", ind2) + "<td class=\"quantTrigaraCenter\">".length();
				if (ind0 > ind1)
					break;
				ind2 = pContingut.indexOf("&nbsp;min.", ind1);
				if (ind1 < 0 || ind2 < 0)
					break;
				String tempsEspera = tractarTempsEspera(pContingut.substring(ind1, ind2));
				if (tempsEspera == null)
					break;
		...
		//
		return arrTempsEspera;
	}

	private static String tractarTempsEspera(String pText) {
		/*
		 * 		<td class="quantTrigaraCenter">
					<img src="DOCUMENTS/Menu/Linies/l5no.png" width="15px">
				</td>
		...
		int ind2 = 0;
		try {
			ind1 = pText.indexOf("title", ind2) + "title".length();
